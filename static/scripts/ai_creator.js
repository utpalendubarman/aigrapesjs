...
<div class="understandings-holder">
  ...
  editor.Commands.add('open-custom-modal', {
    run(editor, sender) {
      ...
      init_ai(modal);
      modal.open();
      document.querySelector('.gjs-mdl-btn-close').style.display = "none"
...
function pushScript(editor,script) {
  editor.addComponents({
    type: "",
    content: "",
    script: function (){
      try {
        new Function(script)(); // Executes the code in a separate scope
      } catch (e) {
...
function extractJs(htmlString){
  ...
  while ((match = scriptRegex.exec(htmlString)) !== null) {
    scripts.push(match[1]); // Push the JavaScript code
    console.log(match[1])
  }
  return scripts;
}
